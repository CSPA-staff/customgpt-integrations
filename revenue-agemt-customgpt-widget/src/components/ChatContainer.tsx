...
import { saveConversation, loadConversation, clearConversation } from '@/hooks/useConversationPersistence';
...
const ChatContainer = ({ onVoiceMode, theme, capabilities }: ChatContainerProps) => {
  ...
  useEffect(() => {
    const initConversation = async () => {
      ...
      // Create new conversation if no valid stored session
      try {
  ...
  const handleRefresh = async () => {
    try {
      ...
      saveConversation(data.session_id);
      console.log('[ChatContainer] Started new conversation:', data.session_id);
    } catch (error) {
